...
class Node():
    ...
    Node class representing a single node in the network.
        data/state:
            ...
            @check_servers_greenlet: Optional[]
    ...
    def __init__(self, addr: str, config_file='server_config'):
        ...
        cprint("My address: {}".format(self.addr), 'magenta')
    ...
    def timeout(self):
        ...
        if self.state is State.Normal or self.state is State.Reorganization:
            try:
                ...
                print("{} Timeout!".format(self.servers[self.coordinator]))
                self.election()
        else:
            self.election()
    # ------------------------------------------------------------------------------------------------------------------
...
def main() -> None:
    ...
    server = SimpleXMLRPCServer(
        ('127.0.0.1', int(addr)), logRequests=True, allow_none=False)
    server.register_instance(Node(addr))
